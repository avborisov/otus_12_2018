...
public class RubleBankNotesAcceptor<T extends RubleBankNote> implements BankNotesAcceptor<T>, Cloneable {

    SortedMap<RubleBankNoteType, BankNoteHolder> holders;
    RubleBankNotesAcceptorMemento defaultState;
    ...
    public void saveDefaultState() {
        defaultState = new RubleBankNotesAcceptorMemento(this);
    }

    public boolean returnToDefaultState() {
        if (defaultState == null) {
            System.err.println("Default state not saved");
            return false;
        }
        holders = defaultState.getHolders();
        return true;
    }

    // For recover state of ATM to default we need to remove old acceptor physically (box with money holders)
    // and input new with default money amount. In our program we need to reset state to default.
    // Default state we can save in memento.
    private class RubleBankNotesAcceptorMemento {

        private SortedMap<RubleBankNoteType, BankNoteHolder> holders;

        public RubleBankNotesAcceptorMemento(RubleBankNotesAcceptor originator) {

            holders = new TreeMap<>(new BankNoteTypeComparator());

            SortedMap<RubleBankNoteType, BankNoteHolder> originalHolders = originator.holders;
            for (RubleBankNoteType key : originalHolders.keySet()) {
                BankNoteHolder holder = originalHolders.get(key);
                BankNoteHolder holderCopy = new BankNoteHolder(holder.getMaxCount());
                for (int i = 0; i < holder.getMoneyCount(); i++) {
                    holderCopy.put();
                }
                holders.put(key, holderCopy);
            }
        }

        public SortedMap<RubleBankNoteType, BankNoteHolder> getHolders() {
            return holders;
        }
    }
